...
INSERT INTO membership_shopping_credit_lot(id,participant_id,source,funding,issued_minor,available_minor,issued_at,source_reference,state) VALUES('credit:a2-over','participant:member','MEMBERSHIP_OVERPAYMENT_CREDIT','MEMBER_FUNDED',1000,1000,now(),'membership-overpayment:settlement:a2-over','AVAILABLE');
SQL
...
if "${PSQL[@]}" -c "INSERT INTO membership_shopping_credit_lot(id,participant_id,source,funding,issued_minor,available_minor,issued_at,source_reference,state) VALUES('credit:a2-bad','participant:member','MEMBERSHIP_OVERPAYMENT_CREDIT','CLUB_FUNDED',1000,1000,now(),'bad:a2','AVAILABLE')" >/dev/null 2>&1; then echo "A2 member-funded overpayment invariant bypass unexpectedly succeeded" >&2; exit 1; fi
if "${PSQL[@]}" -c "INSERT INTO membership_shopping_credit_lot(id,participant_id,source,funding,issued_minor,available_minor,issued_at,source_reference,state) VALUES('credit:a2-replay','participant:member','MEMBERSHIP_OVERPAYMENT_CREDIT','MEMBER_FUNDED',1000,1000,now(),'membership-overpayment:settlement:a2-over','AVAILABLE')" >/dev/null 2>&1; then echo "A2 shopping-credit source replay unexpectedly succeeded" >&2; exit 1; fi
