...
const now='2026-09-16T12:00:00Z',memberId='participant:a11-member';
...
test('A11 verified annual settlement restores commerce eligibility',()=>{const billing=new InMemoryMembershipBillingStore();const invoiceId='invoice:a11-settle';billing.issue({id:invoiceId,participantId:memberId,amountMinor:10000,issuedAt:'2026-09-01T00:00:00Z',dueAt:'2026-09-10T00:00:00Z'});billing.markDue(invoiceId);billing.markPastDue(invoiceId,'2026-09-11T00:00:00Z');billing.recordAuthoritativeSettlement(invoiceId,evidence(invoiceId,10000,'settlement:a11'));const result=authorizeDemandCommitment({request,basket,offer,memberInGoodStanding:billing.standing(memberId,now).state==='ACTIVE',now});assert.equal(result.ok,true);assert.equal(pooledDemandQuantity([result.commitment],offer.offerId),10);});
