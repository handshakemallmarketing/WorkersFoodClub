...
test('INV-013 savings requires governed comparable executable benchmark',()=>{
 const l=new InMemoryEconomicsLedger();
 l.defineBenchmark(benchmark());
...
test('INV-013 actual mandatory member charges are included in comparable outlay',()=>{
 const l=prepared();
 const s=l.calculateSavings({id:'savings:1',benchmarkValuationId:'valuation:rice-1',memberEconomicsId:'member-econ:rice-1',calculatedAt:'2026-09-07T10:06:00Z'});
 assert.equal(s.comparableMemberOutlay.minor,46000n);
 assert.equal(s.absoluteSavings.minor,4000n);
 assert.equal(s.benchmarkRelativeBasisPoints,800n);
});

test('INV-013 zero and negative savings are preserved, never suppressed',()=>{
 const zero=prepared();
 zero.recordFulfilledMemberEconomics(memberEconomics({id:'member-econ:zero',goodsOutlay:money(49000n,'GHS')}));
 ...
 const negative=prepared();
 negative.recordFulfilledMemberEconomics(memberEconomics({id:'member-econ:negative',goodsOutlay:money(51000n,'GHS')}));
 ...
 assert.equal(s.absoluteSavings.minor,-2000n);
 assert.equal(s.benchmarkRelativeBasisPoints,-400n);
});

test('C2 benchmark method must exist before valuation and savings cannot cherry-pick incompatible quantity',()=>{
 const l=new InMemoryEconomicsLedger();
 assert.throws(()=>l.recordBenchmarkValuation(valuation()),/BENCHMARK_METHOD_UNKNOWN/);
 l.defineBenchmark(benchmark());
 l.recordBenchmarkValuation(valuation());
...
test('C6 refund changes actual member economics and forces recomputed savings as a new entry',()=>{
 const l=prepared();
 const first=l.calculateSavings({id:'savings:before-refund',benchmarkValuationId:'valuation:rice-1',memberEconomicsId:'member-econ:rice-1',calculatedAt:'2026-09-07T10:06:00Z'});
 ...
 assert.equal(first.absoluteSavings.minor,4000n);
 assert.equal(corrected.absoluteSavings.minor,9000n);
 assert.equal(l.getSavings(first.id).absoluteSavings.minor,4000n);
});

test('INV-012 cost basis is purpose-specific and fully componentized',()=>{
 const l=prepared();
 const c=l.recordCostBasis({id:'cost:fulfilled-rice',obligationId,purpose:'FULFILLED_MEMBER_OBLIGATION',currency:'GHS',methodVersion:'cost:v1',components:[
  {kind:'ACQUISITION',amount:money(30000n,'GHS'),evidenceIds:[eid('evidence:purchase')]},
  {kind:'INBOUND_LOGISTICS',amount:money(3000n,'GHS'),evidenceIds:[eid('evidence:truck')]},
  {kind:'HANDLING',amount:money(2000n,'GHS'),evidenceIds:[eid('evidence:handling')]},
  {kind:'FINANCING',amount:money(1000n,'GHS'),evidenceIds:[eid('evidence:finance')]},
  {kind:'OUTBOUND_LOGISTICS',amount:money(2000n,'GHS'),evidenceIds:[eid('evidence:outbound')]},
  {kind:'RISK',amount:money(1000n,'GHS'),evidenceIds:[eid('evidence:risk')]}
 ],support:[],calculatedAt:'2026-09-07T10:05:00Z'});
 assert.equal(c.purpose,'FULFILLED_MEMBER_OBLIGATION');
 assert.equal(c.riskAdjustedCost.minor,39000n);
 assert.equal(c.components.length,6);
});

test('INV-014 subsidy and promotion remain separate from structural advantage',()=>{
 const l=prepared();
 const c=l.recordCostBasis({id:'cost:supported-rice',obligationId,purpose:'FULFILLED_MEMBER_OBLIGATION',currency:'GHS',methodVersion:'cost:v1',components:[
  {kind:'ACQUISITION',amount:money(35000n,'GHS'),evidenceIds:[eid('evidence:purchase')]},
  {kind:'OUTBOUND_LOGISTICS',amount:money(5000n,'GHS'),evidenceIds:[eid('evidence:delivery')]}
 ],support:[
  {kind:'SUBSIDY',amount:money(7000n,'GHS'),evidenceIds:[eid('evidence:grant')]},
  {kind:'PROMOTION',amount:money(3000n,'GHS'),evidenceIds:[eid('evidence:promo')]}
 ],calculatedAt:'2026-09-07T10:05:00Z'});
 ...
 assert.equal(a.riskAdjustedSystemCost.minor,40000n);
 assert.equal(a.nonStructuralSupport.minor,10000n);
 assert.equal(a.structuralAdvantage.minor,10000n);
});

test('FX-012 inaccessible or stale benchmark cannot manufacture member savings',()=>{
...
test('FX-013 promotion support cannot be laundered into structural advantage',()=>{
 const l=prepared();
 const c=l.recordCostBasis({id:'cost:promo-heavy',obligationId,purpose:'FULFILLED_MEMBER_OBLIGATION',currency:'GHS',methodVersion:'cost:v1',components:[{kind:'ACQUISITION',amount:money(48000n,'GHS'),evidenceIds:[eid('evidence:purchase')]}],support:[{kind:'PROMOTION',amount:money(20000n,'GHS'),evidenceIds:[eid('evidence:promo')]}],calculatedAt:'2026-09-07T10:05:00Z'});
 ...
 assert.equal(a.structuralAdvantage.minor,2000n);
 assert.equal(a.nonStructuralSupport.minor,20000n);
});
