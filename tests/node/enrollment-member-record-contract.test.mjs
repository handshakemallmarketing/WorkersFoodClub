...
test('settlement cannot mint or replace a Member Number',()=>{assert.match(settleSource,/MEMBER_NUMBER_NOT_ISSUED/);assert.doesNotMatch(settleSource,/randomBytes|randomUUID|WFC-P-/);const membershipUpdates=[...settleSource.matchAll(/UPDATE\s+application_membership\b([\s\S]*?)(?=RETURNING|;|`)/gi)].map(m=>m[1]);for(const update of membershipUpdates)assert.doesNotMatch(update,/\bSET\b[\s\S]*?\bpublic_member_id\s*=/i);});
