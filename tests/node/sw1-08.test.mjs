...
test('SW1-08 economics evidence and component collections are deeply immutable',()=>{
 const economics=new InMemoryEconomicsLedger();
 const benchmark=economics.defineBenchmark({id:'benchmark:immutable',version:1,purpose:'MEMBER_SAVINGS',specificationId:spec,quantity:quantity(5,'kg'),place:'Accra',serviceLevel:'pickup',transactionLevel:'RETAIL',validFrom:'2026-09-01T00:00:00Z',validUntil:'2026-09-30T23:59:59Z',normalizationRuleVersion:'norm:v1',availabilityRuleVersion:'availability:v1',observationEvidenceIds:[eid('e:market')],definedAt:'2026-09-01T00:00:00Z'});
 assert.throws(()=>benchmark.observationEvidenceIds.push(eid('e:tamper')),/TypeError/);
 const valuation=economics.recordBenchmarkValuation({id:'valuation:immutable',benchmarkId:'benchmark:immutable',benchmarkVersion:1,obligationId,specificationId:spec,quantity:quantity(5,'kg'),place:'Accra',serviceLevel:'pickup',availability:'EXECUTABLE',comparableValue:money(1000n,'GHS'),evaluatedAt:'2026-09-09T09:05:00Z',evidenceIds:[eid('e:market')]});
 assert.throws(()=>valuation.evidenceIds.push(eid('e:tamper')),/TypeError/);
 const memberEconomics=economics.recordFulfilledMemberEconomics({id:'member-econ:immutable',obligationId,participantId:member,specificationId:spec,quantity:quantity(5,'kg'),place:'Accra',serviceLevel:'pickup',goodsOutlay:money(800n,'GHS'),mandatoryCharges:money(0n,'GHS'),refundApplied:money(0n,'GHS'),economicEvidenceIds:[eid('e:payment')],realizedAt:'2026-09-09T09:10:00Z',substitutionEvidenceIds:[eid('e:substitution')]});
 assert.throws(()=>memberEconomics.economicEvidenceIds.push(eid('e:tamper')),/TypeError/);assert.throws(()=>memberEconomics.substitutionEvidenceIds.push(eid('e:tamper')),/TypeError/);
 const cost=economics.recordCostBasis({id:'cost:immutable',obligationId,purpose:'FULFILLED_MEMBER_OBLIGATION',currency:'GHS',methodVersion:'cost:v1',components:[{kind:'ACQUISITION',amount:money(600n,'GHS'),evidenceIds:[eid('e:cost')]}],support:[{kind:'SUBSIDY',amount:money(20n,'GHS'),evidenceIds:[eid('e:support')]}],calculatedAt:'2026-09-09T09:11:00Z'});
 assert.throws(()=>cost.components.push({kind:'OTHER',amount:money(1n,'GHS'),evidenceIds:[eid('e:x')]}),/TypeError/);assert.throws(()=>cost.components[0].evidenceIds.push(eid('e:tamper')),/TypeError/);assert.throws(()=>cost.support.push({kind:'GRANT',amount:money(1n,'GHS'),evidenceIds:[eid('e:x')]}),/TypeError/);assert.throws(()=>cost.support[0].evidenceIds.push(eid('e:tamper')),/TypeError/);
 const savings=economics.calculateSavings({id:'savings:immutable',benchmarkValuationId:'valuation:immutable',memberEconomicsId:'member-econ:immutable',calculatedAt:'2026-09-09T09:20:00Z'});
 assert.throws(()=>savings.benchmarkEvidenceIds.push(eid('e:tamper')),/TypeError/);assert.throws(()=>savings.memberEconomicEvidenceIds.push(eid('e:tamper')),/TypeError/);
});
