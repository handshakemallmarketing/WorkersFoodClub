...
const pid=x=>asId(x);const eid=x=>asId(x);
const auth={participantId:pid('participant:board'),authorityGrantId:'grant:transfer',scope:'TRANSFER_POLICY_GOVERNANCE',validFrom:'2026-09-01T00:00:00Z'};
...
test('INV-019 title and risk are orthogonal and may transfer at different events',()=>{
 const r=new GovernedTransferPolicyRegistry();r.ratify(policy(),auth);const e=new GovernedTransferEvaluator(r);
...
test('RC1-B04 transaction requires an explicitly ratified policy version',()=>{
 const r=new GovernedTransferPolicyRegistry();const e=new GovernedTransferEvaluator(r);
...
test('RC1-B04 unauthorized or revoked authority cannot ratify transfer policy',()=>{
 const r=new GovernedTransferPolicyRegistry();
 assert.throws(()=>r.ratify(policy(),{...auth,scope:'OTHER'}),/TRANSFER_POLICY_UNAUTHORIZED/);
 assert.throws(()=>r.ratify(policy(),{...auth,revokedAt:'2026-09-08T02:59:59Z'}),/TRANSFER_POLICY_AUTHORITY_REVOKED/);
});

test('INV-024 policy versions are immutable and sequential rather than silently rewritten',()=>{
 const r=new GovernedTransferPolicyRegistry();r.ratify(policy(),auth);
 assert.throws(()=>r.ratify(policy(),auth),/TRANSFER_POLICY_VERSION_DUPLICATE/);
 ...
 r.ratify(v2,auth);assert.equal(r.get('policy:pickup',1).title.trigger,'ACCEPTANCE');assert.equal(r.get('policy:pickup',2).title.trigger,'NAMED_EVENT');
});

test('INV-019 named policy trigger proves no universal delivered_at rule exists',()=>{
 const r=new GovernedTransferPolicyRegistry();r.ratify(policy({title:{dimension:'TITLE',trigger:'NAMED_EVENT',eventType:'TITLE_RELEASE'},risk:{dimension:'RISK',trigger:'ACCEPTANCE'}}),auth);const e=new GovernedTransferEvaluator(r);
