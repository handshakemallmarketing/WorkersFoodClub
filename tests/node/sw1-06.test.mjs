...
async function setup(){
 ...
 const resolution=new InMemoryObligationResolutionLedger();const fulfillment=new InMemoryFulfillmentLedger(resolution);const remedies=new InMemoryRemedyLedger(resolution);const authority=new AuthorityEvaluator(authorityStore);
 const service=new GovernedPilotFulfillmentService(authority,inventory,demand,fulfillment,remedies);
 return {service,remedies,fulfillment,resolution,inventory,demand,authority,warehouseGrant,memberGrant,riceLot};
}
...
test('SW1-06 shortfall cannot exceed unresolved quantity or detach from acceptance evidence',async()=>{const {service,warehouseGrant,memberGrant}=await ready();service.handover({actorId:warehouse,grantIds:[warehouseGrant],at},handover());service.accept({actorId:member,grantIds:[memberGrant],at},{id:'acceptance:partial2',handoverId:'handover:1',obligationId,participantId:member,state:'PARTIALLY_ACCEPTED',quantity:quantity(4,'kg'),acceptedAt:'2026-09-09T01:31:00Z',evidenceIds:[eid('evidence:acceptance-partial2')]});assert.throws(()=>service.recordException({actorId:member,grantIds:[memberGrant],at},{id:'exception:missing-link',obligationId,participantId:member,kind:'SHORTFALL',affectedQuantity:quantity(1,'kg'),occurredAt:'2026-09-09T01:32:00Z',evidenceIds:[eid('evidence:shortfall')]}),/EXCEPTION_ACCEPTANCE_LINK_REQUIRED/);assert.throws(()=>service.recordException({actorId:member,grantIds:[memberGrant],at},{id:'exception:too-large',obligationId,participantId:member,kind:'SHORTFALL',affectedQuantity:quantity(2,'kg'),occurredAt:'2026-09-09T01:32:00Z',evidenceIds:[eid('evidence:shortfall2')],relatedAcceptanceId:'acceptance:partial2'}),/EXCEPTION_ACCEPTANCE_DEFICIT_MISMATCH|EXCEPTION_EXCEEDS_UNRESOLVED_QUANTITY/);});

test('review debt: fulfillment and remedy ledgers must share one resolution ledger',async()=>{const x=await setup();assert.throws(()=>new GovernedPilotFulfillmentService(x.authority,x.inventory,x.demand,new InMemoryFulfillmentLedger(new InMemoryObligationResolutionLedger()),new InMemoryRemedyLedger(new InMemoryObligationResolutionLedger())),/FULFILLMENT_RESOLUTION_LEDGER_MISMATCH/);});

test('review debt: fulfillment requires exact state transitions',async()=>{const {service,warehouseGrant,riceLot}=await setup();const ctx={actorId:warehouse,grantIds:[warehouseGrant],at};service.recordWork(ctx,work('PICKED','pick:strict',undefined,riceLot));assert.throws(()=>service.recordWork(ctx,work('READY_FOR_PICKUP','ready:jump','pick:strict',riceLot)),/FULFILLMENT_STATE_TRANSITION_INVALID/);});

test('review debt: a later quarantine blocks fulfillment of an allocated lot',async()=>{const {service,inventory,warehouseGrant,riceLot}=await setup();inventory.assessQuality({id:'quality:2',lotId:riceLot,state:'QUARANTINED',assessedAt:'2026-09-09T01:10:00Z',evidenceIds:[eid('evidence:quarantine')],supersedes:'quality:1'});assert.throws(()=>service.recordWork({actorId:warehouse,grantIds:[warehouseGrant],at},work('PICKED','pick:unsafe',undefined,riceLot)),/FULFILLMENT_LOT_NOT_CURRENTLY_ACCEPTED/);});

test('review debt: acceptance state must match the accepted quantity',async()=>{const {service,warehouseGrant,memberGrant}=await ready();service.handover({actorId:warehouse,grantIds:[warehouseGrant],at},handover());const base={handoverId:'handover:1',obligationId,participantId:member,acceptedAt:'2026-09-09T01:31:00Z',evidenceIds:[eid('evidence:acceptance-state')]};assert.throws(()=>service.accept({actorId:member,grantIds:[memberGrant],at},{...base,id:'accept:bad-partial',state:'PARTIALLY_ACCEPTED',quantity:quantity(5,'kg')}),/PARTIAL_ACCEPTANCE_QUANTITY_INVALID/);assert.throws(()=>service.accept({actorId:member,grantIds:[memberGrant],at},{...base,id:'accept:bad-full',state:'ACCEPTED',quantity:quantity(4,'kg')}),/ACCEPTED_ACCEPTANCE_QUANTITY_MISMATCH/);});

test('review debt: cumulative shortfall claims cannot exceed one unresolved deficit',async()=>{const {service,warehouseGrant,memberGrant}=await ready();service.handover({actorId:warehouse,grantIds:[warehouseGrant],at},handover());service.accept({actorId:member,grantIds:[memberGrant],at},{id:'acceptance:cumulative',handoverId:'handover:1',obligationId,participantId:member,state:'PARTIALLY_ACCEPTED',quantity:quantity(4,'kg'),acceptedAt:'2026-09-09T01:31:00Z',evidenceIds:[eid('evidence:acceptance-cumulative')]});const ctx={actorId:member,grantIds:[memberGrant],at};service.recordException(ctx,{id:'exception:first',obligationId,participantId:member,kind:'SHORTFALL',affectedQuantity:quantity(1,'kg'),occurredAt:'2026-09-09T01:32:00Z',evidenceIds:[eid('evidence:first')],relatedAcceptanceId:'acceptance:cumulative'});assert.throws(()=>service.recordException(ctx,{id:'exception:second',obligationId,participantId:member,kind:'SHORTFALL',affectedQuantity:quantity(1,'kg'),occurredAt:'2026-09-09T01:33:00Z',evidenceIds:[eid('evidence:second')],relatedAcceptanceId:'acceptance:cumulative'}),/EXCEPTION_EXCEEDS_UNRESOLVED_QUANTITY/);});

test('review debt: fulfillment chronology cannot predate allocation',async()=>{const {service,warehouseGrant,riceLot}=await setup();const early={...work('PICKED','pick:early',undefined,riceLot),occurredAt:'2026-09-09T01:06:30Z'};assert.throws(()=>service.recordWork({actorId:warehouse,grantIds:[warehouseGrant],at},early),/FULFILLMENT_CAUSAL_TIME_INVALID/);});
