...
export class GovernedMemberEconomicsService{
 ...
 recordGovernedSavingsBenchmark(input:{benchmarkId:string;benchmarkVersion:number;valuationId:string;obligationId:ObligationId;listingId:string;offerId:any;benchmarkDisplayId:string}){
  ...
  const methodInput={id:input.benchmarkId,version:input.benchmarkVersion,purpose:'MEMBER_SAVINGS' as const,specificationId:benchmark.specificationId,quantity:quantity as Quantity,place:benchmark.place,serviceLevel:'pickup',transactionLevel:benchmark.transactionLevel,validFrom:view.offer.validFrom,validUntil:view.offer.validUntil,normalizationRuleVersion:benchmark.methodVersion,availabilityRuleVersion:'catalog-offer-executable-v1',observationEvidenceIds:[benchmark.priceEvidenceId],definedAt:commitment.acceptedAt};
  const valuationInput={id:input.valuationId,benchmarkId:input.benchmarkId,benchmarkVersion:input.benchmarkVersion,obligationId:input.obligationId,specificationId:benchmark.specificationId,quantity:quantity as Quantity,place:benchmark.place,serviceLevel:'pickup',availability:'EXECUTABLE' as const,comparableValue:Object.freeze({minor:comparableMinor,currency:benchmark.value.currency}),evaluatedAt:commitment.acceptedAt,evidenceIds:[benchmark.priceEvidenceId]};
