...
export interface VerifiedMembershipPaymentEvidence {
 readonly evidenceId:string; readonly providerReference:string; readonly participantId:ParticipantId; readonly obligationId:string;
 readonly amountMinor:number; readonly verified:true; readonly persisted:true; readonly verifiedAt:string;
}
...
export class InMemoryMembershipBillingStore {
 ...
 standing(participantId:ParticipantId,restricted:boolean):MembershipStanding{const pastDue=[...this.invoices.values()].filter(x=>x.participantId===participantId&&x.state==='PAST_DUE').map(x=>x.id);return Object.freeze({participantId,state:restricted&&pastDue.length?'RESTRICTED':pastDue.length?'PAST_DUE':'CURRENT',basisInvoiceIds:Object.freeze(pastDue)});}
