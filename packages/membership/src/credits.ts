...
export type ShoppingCreditSource='MEMBERSHIP_FEE_SPENDING_CREDIT'|'PROMOTIONAL_CREDIT'|'MEMBERSHIP_OVERPAYMENT_CREDIT';
...
export class InMemoryShoppingCreditStore {
 ...
 issue(input:{id:string;participantId:ParticipantId;source:ShoppingCreditSource;funding:ShoppingCreditFunding;amountMinor:number;issuedAt:string;expiresAt?:string;sourceReference:string;campaignReference?:string}):ShoppingCreditLot{
 ...
 issueMembershipFeeSpendingCredit(input:{id:string;participantId:ParticipantId;annualFeeMinor:number;issuedAt:string;sourceReference:string}):ShoppingCreditLot{
  ...
  return this.issue({id:input.id,participantId:input.participantId,source:'MEMBERSHIP_FEE_SPENDING_CREDIT',funding:'CLUB_FUNDED',amountMinor:input.annualFeeMinor,issuedAt:input.issuedAt,expiresAt:expiry.toISOString(),sourceReference:input.sourceReference});
 }
 issuePromotionalCredit(input:{id:string;participantId:ParticipantId;amountMinor:number;issuedAt:string;expiresAt?:string;sourceReference:string;campaignReference:string}):ShoppingCreditLot{
  return this.issue({...input,source:'PROMOTIONAL_CREDIT',funding:'CLUB_FUNDED'});
 }
