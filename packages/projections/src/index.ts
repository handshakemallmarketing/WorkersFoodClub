...
export class CanonicalRecordLog{
 private readonly records:CanonicalProjectionRecord[]=[];
 append<T>(record:CanonicalProjectionRecord<T>):CanonicalProjectionRecord<T>{
  ...
  if(this.records.some(x=>x.recordId===record.recordId)) throw new Error('CANONICAL_RECORD_ID_DUPLICATE');
  if(this.records.some(x=>x.sequence===record.sequence)) throw new Error('CANONICAL_SEQUENCE_DUPLICATE');
  const frozen=Object.freeze({...record});this.records.push(frozen);return frozen;
 }
...
export interface FulfillmentOperationalView{readonly obligationId:string;readonly acceptedQuantity:number;readonly state:'OPEN'|'PARTIALLY_DISCHARGED'|'DISCHARGED';readonly sourceRecordIds:readonly string[];}
...
export const fulfillmentProjection:ProjectionDefinition<FulfillmentOperationalView>={name:'fulfillment-operational',key:r=>{const p=r.payload as FulfillmentPayload;return p.kind==='OBLIGATION_OPENED'||p.kind==='ACCEPTANCE'?p.obligationId:undefined;},reduce:(c,r)=>{const p=r.payload as FulfillmentPayload;if(p.kind==='OBLIGATION_OPENED')return Object.freeze({obligationId:p.obligationId,acceptedQuantity:0,state:'OPEN' as const,sourceRecordIds:[r.recordId],_required:p.quantity} as FulfillmentOperationalView&{_required:number});if(!c)throw new Error('PROJECTION_OBLIGATION_LINEAGE_MISSING');const required=(c as FulfillmentOperationalView&{_required?:number})._required;if(required===undefined)throw new Error('PROJECTION_REQUIRED_QUANTITY_MISSING');const accepted=c.acceptedQuantity+p.quantity;if(accepted>required)throw new Error('PROJECTION_OVERDISCHARGE');return Object.freeze({...c,acceptedQuantity:accepted,state:accepted===0?'OPEN':accepted===required?'DISCHARGED':'PARTIALLY_DISCHARGED',sourceRecordIds:[...c.sourceRecordIds,r.recordId]});}};
