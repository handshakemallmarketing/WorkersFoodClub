...
export interface PurchaseAuthorizationTerms {readonly participantId:ParticipantId;readonly action:'AcceptMemberPurchase';readonly cartId:string;readonly offerId:OfferId;readonly quantity:Quantity;}
export interface AcceptedCommandVerifier {isAccepted(commandId:string,eventId:string,expected?:PurchaseAuthorizationTerms):boolean|Promise<boolean>;}
...
export class InMemoryDemandCommitmentLedger {
 ...
 async commitPurchase(input:{obligationId:ObligationId;participantId:ParticipantId;membership:MembershipRelationship;offer:MemberOffer;quantity:Quantity;sourceDemandSignalId?:string;authorizedCommandId:CommandId;authorizedEventId:string;acceptedAt:string;policyVersions:readonly string[];authorizationTerms?:PurchaseAuthorizationTerms}):Promise<PurchaseCommitmentRecord>{
  ...
  if(input.authorizationTerms){if(input.authorizationTerms.participantId!==input.participantId||input.authorizationTerms.offerId!==input.offer.id||input.authorizationTerms.quantity.amount!==input.quantity.amount||input.authorizationTerms.quantity.unit!==input.quantity.unit) throw new Error('AUTHORIZED_COMMITMENT_TERMS_MISMATCH');}
  ...
  try{if(!(await this.commandVerifier.isAccepted(String(input.authorizedCommandId),input.authorizedEventId,input.authorizationTerms))) throw new Error('AUTHORIZED_COMMITMENT_NOT_VERIFIED');}
