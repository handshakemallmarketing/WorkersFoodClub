...
export class InMemoryDemandCommitmentLedger {
 ...
 async commitPurchase(input:{obligationId:ObligationId;participantId:ParticipantId;membership:MembershipRelationship;offer:MemberOffer;quantity:Quantity;sourceDemandSignalId?:string;authorizedCommandId:CommandId;authorizedEventId:string;acceptedAt:string;policyVersions:readonly string[]}):Promise<PurchaseCommitmentRecord>{
  ...
  const record:PurchaseCommitmentRecord=Object.freeze({obligation,participantId:input.participantId,membershipId:input.membership.id,offerId:input.offer.id,committedMemberPrice:Object.freeze({...input.offer.memberPrice}),committedPriceBasis:Object.freeze({...input.offer.priceBasis}),...(input.sourceDemandSignalId!==undefined?{sourceDemandSignalId:input.sourceDemandSignalId}:{}),authorizedCommandId:input.authorizedCommandId,authorizedEventId:input.authorizedEventId,acceptedAt:input.acceptedAt,policyVersions:Object.freeze([...input.policyVersions])});
  this.commitments.set(input.obligationId,record);return record;
 }
 recordPaymentEvidence(payment:PaymentEvidenceRecord):PaymentEvidenceRecord{if(this.payments.has(payment.evidenceId)) throw new Error('PAYMENT_EVIDENCE_DUPLICATE');const providerKey=`${payment.provider}:${payment.providerReference}`;if(this.providerRefs.has(providerKey)) throw new Error('PAYMENT_PROVIDER_REFERENCE_DUPLICATE');if(!this.commitments.has(payment.obligationId)) throw new Error('PAYMENT_OBLIGATION_UNKNOWN');if(!payment.provider.trim()||!payment.providerReference.trim()) throw new Error('PAYMENT_PROVIDER_REFERENCE_REQUIRED');if(!validTime(payment.observedAt)||!validTime(payment.recordedAt)) throw new Error('PAYMENT_TIME_INVALID');const frozen=Object.freeze({...payment,amount:Object.freeze({...payment.amount})});this.payments.set(payment.evidenceId,frozen);this.providerRefs.set(providerKey,payment.evidenceId);return frozen;}
