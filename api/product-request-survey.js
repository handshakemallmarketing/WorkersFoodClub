...
export default async function handler(req,res){
 ...
 try{
  ...
  if(prior[0]){if(String(prior[0].subject)!==subject)return res.status(409).json({ok:false,error:'SURVEY_REQUEST_REBOUND'});return res.status(200).json({ok:true,survey:serialize(prior[0],true)});}
  ...
  if(error?.code==='23505'){try{const {neon}=await import('@neondatabase/serverless');const sql=neon(connectionString);const prior=await sql`SELECT * FROM member_product_request_survey WHERE participant_id=${principal.actorId} AND idempotency_key=${requestId} LIMIT 1`;if(prior[0]){if(String(prior[0].subject)!==subject)return res.status(409).json({ok:false,error:'SURVEY_REQUEST_REBOUND'});return res.status(200).json({ok:true,survey:serialize(prior[0],true)});}}catch{}}
