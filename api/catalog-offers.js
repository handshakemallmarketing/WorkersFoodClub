...
export default async function handler(req,res){
 ...
 try{
  ...
  if(req.method==='DELETE'){
   const rows=await sql`UPDATE preview_member_offer SET status='CLOSED',updated_at=now() WHERE offer_id=${offerId} RETURNING offer_id,status,updated_at`;
   if(!rows[0])return res.status(404).json({ok:false,error:'OFFER_NOT_FOUND'});return res.status(200).json({ok:true,offer:{offerId,status:'CLOSED'}});
  }
  ...
  const rows=await sql`UPDATE preview_member_offer SET name=${name},description=${description},unit_quantity=${unitQuantity},unit=${unit},price_minor=${priceMinor},currency='GHS',fulfillment_method=${method},status=${status},sort_order=${sort},min_order_quantity=${min},max_order_quantity=${max},campaign_capacity=${capacity},updated_at=now() WHERE offer_id=${offerId} RETURNING offer_id,status`;
