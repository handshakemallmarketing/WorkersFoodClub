 ...
 * "verify again" step: it requires a fresh production identity and an existing
 * active authority grant. Workforce authority is deliberately independent of
 * paid membership-invoice standing; employee sponsorship is handled by the
 * membership entitlement lifecycle rather than by this authorization boundary.
 */
export default async function handler(req, res, options = {}) {
  ...
  try {
    ...
    const sessionId = generateSessionId(); const nowMs = options.now ?? Date.now(); const issuedAt = new Date(nowMs); const expiresAt = new Date(nowMs + EMPLOYEE_SESSION_TTL_SECONDS * 1000); const stepUpTokenIat = new Date(verified.principal.issuedAt * 1000);
