...
/**
 * Internal/provider-evidence boundary for annual subscription settlement.
 * This endpoint does not charge money. It consumes already-verified settlement
 * evidence and activates/reactivates only the membership tied to the exact invoice.
 */
export default async function handler(req, res, options = {}) {
  ...
  try {
    ...
    const invoices=await sql`SELECT i.invoice_id,i.membership_id,i.state,i.settlement_evidence_id,m.participant_id,m.state AS membership_state,m.standing FROM membership_subscription_invoice i JOIN application_membership m ON m.membership_id=i.membership_id WHERE i.invoice_id=${invoiceId} LIMIT 1`;
    ...
    if(String(invoice.state)==='PAID'){
      ...
      res.setHeader('Cache-Control','no-store'); return res.status(200).json({ok:true,idempotent:true,invoiceId,membershipId:String(invoice.membership_id),membershipState:'ACTIVE',standing:'CURRENT'});
    }
    ...
    await sql`UPDATE application_membership SET state='ACTIVE',standing='CURRENT',activated_at=COALESCE(activated_at,${nowIso}),suspended_at=NULL WHERE membership_id=${String(invoice.membership_id)} AND state IN ('INACTIVE','SUSPENDED')`;
    ...
    res.setHeader('Cache-Control','no-store'); return res.status(200).json({ok:true,idempotent:false,invoiceId,membershipId:String(invoice.membership_id),membershipState:'ACTIVE',standing:'CURRENT'});
