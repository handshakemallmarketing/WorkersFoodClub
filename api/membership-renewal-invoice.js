...
/** Renewal billing follows the canonical standing lifecycle: INITIAL_FEE_DUE is initial enrollment; ENDED is terminal. ACTIVE members may be invoiced for a new subscription year before delinquency; GRACE/RESTRICTED/SUSPENDED remain renewable. */
export default async function handler(req,res,options={}){
 ...
 try{let sql=options.sql;if(!sql){const{neon}=await import('@neondatabase/serverless');sql=neon(connectionString,{fetchOptions:{signal:AbortSignal.timeout(5000)}});}
  ...
  const standing=String(member.standing);if(!['ACTIVE','GRACE','RESTRICTED','SUSPENDED'].includes(standing))return res.status(409).json({ok:false,error:'MEMBERSHIP_NOT_RENEWAL_DUE'});
