  ...
  try {
   ...
   LEFT JOIN preview_refund_remedy r ON r.obligation_id = c.obligation_id
    ...
    const orders = rows.map((r) => ({
      ...
      unit: String(r.unit),
      pickupPlace: PREVIEW_PICKUP_PLACE,
      paymentStatus: String(r.payment_status),
      paymentEventId: String(r.payment_event_id),
      fulfillment: r.fulfillment_id ? {
        fulfillmentId: String(r.fulfillment_id),
        state: String(r.fulfillment_state),
        readyAt: r.ready_at ? String(r.ready_at) : null,
        acceptedQuantity: r.accepted_quantity === null ? null : Number(r.accepted_quantity),
        shortfallQuantity: r.shortfall_quantity === null ? null : Number(r.shortfall_quantity),
      } : null,
      exception: r.exception_id ? {
        exceptionId: String(r.exception_id),
        kind: String(r.exception_kind),
        affectedQuantity: Number(r.affected_quantity),
      } : null,
      remedy: r.remedy_id ? {
        remedyId: String(r.remedy_id),
        status: String(r.remedy_status),
        amountMinor: Number(r.remedy_amount_minor),
        currency: String(r.remedy_currency),
        authorizeEventId: String(r.authorize_event_id),
        completionEventId: r.completion_event_id ? String(r.completion_event_id) : null,
        providerReference: r.refund_provider_reference ? String(r.refund_provider_reference) : null,
      } : null,
    }));
