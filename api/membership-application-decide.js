...
/**
 * An Owner or Admin approves or rejects a pending membership_application.
 * Approval is the only place a real, ACTIVE application_membership row gets
 * created from a self-service application -- the application itself never
 * grants membership by existing. Requires the same fresh-OIDC-plus-
 * employee-session proof as the authority invite/revoke endpoints, since
 * this creates a real, durable membership record.
 */
export default async function handler(req, res, options = {}) {
  ...
  if (decision !== 'APPROVE' && decision !== 'REJECT') {
    return res.status(400).json({ ok: false, error: 'DECISION_MUST_BE_APPROVE_OR_REJECT' });
  }
  ...
  try {
    ...
    if (tierOf(callerActions) === 'OPERATOR') {
      return res.status(403).json({ ok: false, error: 'MEMBERSHIP_DECISION_REQUIRES_OWNER_OR_ADMIN' });
    }

    const applications = await sql`
      SELECT application_id, issuer, subject, state
    ...
    if (String(application.state) !== 'SUBMITTED') {
      return res.status(409).json({ ok: false, error: 'APPLICATION_NOT_DECIDABLE' });
    }
    ...
    if (decision === 'REJECT') {
      const rejected = await sql`
        UPDATE membership_application
           SET state='REJECTED', decided_at=${nowIso}, decided_by=${callerId}
         WHERE application_id=${applicationId} AND state='SUBMITTED'
    ...
    const existingBindings = await sql`
      SELECT participant_id, state
        FROM application_identity_binding
       WHERE issuer=${applicantIssuer} AND subject=${applicantSubject}
       LIMIT 2`;
    ...
    if (existingBindings.length === 1 && String(existingBindings[0].state) !== 'ACTIVE') {
      return res.status(403).json({ ok: false, error: 'APPLICANT_PRINCIPAL_DISABLED' });
    }
    ...
    if (isNewParticipant) {
      ...
      await sql`
        ...
        VALUES (
          ${bindingId}, ${applicantIssuer}, ${applicantSubject}, ${participantId},
          ${MEMBER_SCOPES}, 'ACTIVE',
          ${`evidence:membership-application-decide:${applicationId}`}, ${nowIso}, ${callerId}, NULL
        )`;
    } else {
      await sql`
        UPDATE application_identity_binding
           SET scopes=ARRAY(SELECT DISTINCT unnest(scopes || ${MEMBER_SCOPES})), updated_at=${nowIso}
         WHERE issuer=${applicantIssuer} AND subject=${applicantSubject}`;
    }
    ...
    await sql`
      ...
      VALUES (${membershipId}, ${participantId}, 'ACTIVE', ${nowIso}, 'membership-application-v1', ${[applicationId]})`;

    const approved = await sql`
      UPDATE membership_application
         SET state='APPROVED', decided_at=${nowIso}, decided_by=${callerId}, resulting_membership_id=${membershipId}
       WHERE application_id=${applicationId} AND state='SUBMITTED'
      RETURNING application_id`;
    ...
    if (approved.length !== 1) {
      await sql`
        UPDATE application_membership
           SET state='ENDED', ended_at=${nowIso}
         WHERE membership_id=${membershipId} AND state='ACTIVE'`;
      return res.status(409).json({ ok: false, error: 'APPLICATION_ALREADY_DECIDED' });
    }

    res.setHeader('Cache-Control', 'no-store');
    return res.status(200).json({ ok: true, applicationId, decision: 'APPROVE', participantId, membershipId });
  } catch (error) {
