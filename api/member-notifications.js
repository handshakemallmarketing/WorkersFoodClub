...
export default async function handler(req, res) {
  ...
  try {
    ...
    return res.status(200).json({ ok: true, source: 'neon-server', memberId: participantId, notifications });
  } catch (error) {
    console.error('Member notification query failed', { name: error?.name, message: error?.message });
    return res.status(503).json({ ok: false, error: 'MEMBER_NOTIFICATIONS_FETCH_FAILED' });
  }
}
