...
/**
 * Self-service guest enrollment creates the membership identity only.
 * Subscription pricing/invoicing is a downstream concern and MUST NOT block
 * collection of prospect information, member-number issuance, or persistence.
 */
export default async function handler(req,res,options={}){
 ...
 try{let sql=options.sql;if(!sql){const{neon}=await import('@neondatabase/serverless');sql=neon(connectionString,{fetchOptions:{signal:AbortSignal.timeout(5000)}});}
  ...
  const rows=await sql`WITH app AS (INSERT INTO membership_application(application_id,issuer,subject,full_name,email,phone,government_employer,applicant_reference,contact_note,state,review_mode,review_status,decided_at) VALUES (${applicationId},NULL,NULL,${fullName},${email},${phone},${governmentEmployer},${applicantReference},${contactNote},'SUBMITTED','AUTO','NOT_REQUIRED',NULL) RETURNING application_id), participant AS (INSERT INTO application_participant(participant_id,kind,state) SELECT ${participantId},'PERSON','ACTIVE' FROM app RETURNING participant_id), membership AS (INSERT INTO application_membership(membership_id,participant_id,state,member_type,standing,established_at,eligibility_policy_version,eligibility_evidence_ids,public_member_id) SELECT ${membershipId},participant_id,'INACTIVE','PRIMARY','INITIAL_FEE_DUE',${nowIso},'self-service-membership-v2',ARRAY[${applicationId}],${publicMemberId} FROM participant RETURNING membership_id,public_member_id), linked AS (UPDATE membership_application SET resulting_membership_id=${membershipId} WHERE application_id=${applicationId} AND EXISTS(SELECT 1 FROM membership) RETURNING application_id) SELECT application_id FROM linked`;
  if(rows.length!==1)throw Object.assign(new Error('MEMBERSHIP_RECORD_CREATION_FAILED'),{code:'MEMBERSHIP_RECORD_CREATION_FAILED'});
  res.setHeader('Cache-Control','no-store');return res.status(201).json({ok:true,applicationId,applicantReference,state:'SUBMITTED',reviewRequired:false,membershipProvisioned:true,membershipId,publicMemberId,membershipState:'INACTIVE',standing:'INITIAL_FEE_DUE',subscriptionInvoicePending:true,loginIdentityBound:false});
