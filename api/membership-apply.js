...
/**
 * A Google identity with no qualifying membership submits a request to join.
 * This never creates membership by itself -- only
 * api/membership-application-decide.js (an Owner/Admin action) does that.
 * Idempotent: a second submission while one is already SUBMITTED returns
 * the existing application rather than creating a duplicate.
 */
export default async function handler(req, res, options = {}) {
  ...
  const contactNote = typeof contactNoteRaw === 'string' && contactNoteRaw.trim().length > 0
    ? contactNoteRaw.trim().slice(0, 2000)
    : null;
  ...
  try {
    ...
    if (bindings.length === 1 && String(bindings[0].state) === 'ACTIVE') {
      ...
      const memberships = await sql`
        SELECT state
         ...
         LIMIT 1`;
      if (memberships.length === 1 && ['ACTIVE', 'SUSPENDED'].includes(String(memberships[0].state))) {
    ...
    if (existing.length === 1) {
      ...
      return res.status(200).json({ ok: true, applicationId: String(existing[0].application_id), state: 'SUBMITTED', alreadySubmitted: true });
    }

    const applicationId = `application:${randomUUID()}`;
    await sql`
      INSERT INTO membership_application(application_id, issuer, subject, contact_note)
      VALUES (${applicationId}, ${verified.principal.issuer}, ${verified.principal.subject}, ${contactNote})`;
