...
export default async function handler(req,res,options={}){
  ...
  try{
    ...
    if(bindings.length!==1||String(bindings[0].state)!=='ACTIVE'){
      ...
      const application=applications.length===1?applications[0]:null;const pending=application!=null&&String(application.state)==='SUBMITTED';
    ...
    const participantId=String(bindings[0].participant_id);let membership=null,invoice=null;
    ...
    if(memberships.length===1){membership=memberships[0];if(String(membership.state)!=='ACTIVE'||!['ACTIVE','GRACE'].includes(String(membership.standing))){const invoices=await sql`SELECT invoice_id,state,due_at FROM membership_subscription_invoice WHERE membership_id=${String(membership.membership_id)} AND state='OPEN' ORDER BY created_at DESC LIMIT 1`;if(invoices.length===1)invoice=invoices[0];}}
    const applications=await sql`SELECT application_id,state,submitted_at FROM membership_application WHERE issuer=${verified.principal.issuer} AND subject=${verified.principal.subject} ORDER BY submitted_at DESC LIMIT 1`;const application=applications.length===1?applications[0]:null;
    ...
    if(membershipState==='ACTIVE'&&standing==='ACTIVE'){accessState='ACTIVE';memberAccessAvailable=true;route='MEMBER';}
    else if(membershipState==='ACTIVE'&&standing==='GRACE'){accessState='GRACE';memberAccessAvailable=true;route='MEMBER';}
    else if(membershipState==='INACTIVE'&&standing==='INITIAL_FEE_DUE'){accessState='INACTIVE_INITIAL_FEE_DUE';route='SUBSCRIPTION_DUE';}
    else if(['RESTRICTED','SUSPENDED','TERMINATED'].includes(standing)||['SUSPENDED','ENDED'].includes(membershipState)){accessState=`MEMBERSHIP_${standing||membershipState}`;route=standing==='SUSPENDED'?'SUBSCRIPTION_DUE':'RESTRICTED';}
