...
function sameCommand(row,c){return (row.participant_id??null)===(c.participantId??null)&&String(row.subject_type)===c.subjectType&&String(row.subject_id)===c.subjectId&&String(row.category)===c.category&&String(row.reason_code)===c.reasonCode&&String(row.created_by_authn_subject_ref)===c.authnSubjectRef&&canonicalJson(row.evidence_refs??[])===canonicalJson(c.evidenceRefs);}
...
export default async function handler(req,res){
 ...
 const authnSubjectRef=`preview-auth:${principal.subject}`;const command={participantId,subjectType,subjectId,category,reasonCode,authnSubjectRef,evidenceRefs};
 ...
 try{const {neon}=await import('@neondatabase/serverless');sql=neon(connectionString,{fetchOptions:{signal:AbortSignal.timeout(5000)}});const prior=await sql`SELECT * FROM support_case WHERE created_by_actor_id=${principal.actorId} AND command_idempotency_key=${requestId} LIMIT 1`;
  ...
  const caseId=durableId('case');const rows=await sql`INSERT INTO support_case(case_id,participant_id,subject_type,subject_id,category,reason_code,state,state_version,evidence_refs,created_by_actor_id,created_by_authn_subject_ref,updated_by_actor_id,updated_by_authn_subject_ref,command_idempotency_key) VALUES(${caseId},${command.participantId},${subjectType},${subjectId},${category},${reasonCode},'OPEN',1,${JSON.stringify(command.evidenceRefs)}::jsonb,${principal.actorId},${authnSubjectRef},${principal.actorId},${authnSubjectRef},${requestId}) RETURNING *`;
