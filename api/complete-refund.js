 ...
 try{
  ...
  const rows=await sql`
   WITH target AS (
    ...
    SELECT ${requestId},${commandId},'COMMITTED',${ownerToken},now(),1,jsonb_build_object('status','COMPLETED','obligationId',obligation_id,'remedyId',remedy_id,'eventIds',jsonb_build_array(${eventId}::text)),now(),now() FROM upd RETURNING command_id
   ), ver AS (
    ...
    SELECT ${eventId},u.obligation_id,v.version,'REMEDY_COMPLETED',jsonb_build_object('operatorId',${runtime.actorId}::text,'remedyId',u.remedy_id,'sourceExceptionId',u.source_exception_id,'kind',u.kind,'quantity',jsonb_build_object('amount',u.quantity,'unit',u.unit),'amount',jsonb_build_object('minor',u.amount_minor,'currency',u.currency),'provider',u.provider,'providerReference',u.provider_reference,'authorizedCommandId',${commandId}::text,'environment',${runtime.environment}::text),u.completed_at FROM upd u JOIN ver v ON v.aggregate_id=u.obligation_id RETURNING event_id
   ), obligation_done AS (
