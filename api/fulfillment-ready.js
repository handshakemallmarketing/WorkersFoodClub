...
export default async function handler(req,res){
 ...
 const runtime=canonicalRuntimeMetadata({principal,environment});
 const ownerToken=runtimeOwnerToken(environment);
 ...
 try{
  ...
  const fulfillmentId=durableId('fulfillment'),eventId=durableId('event'),commandId=durableId('command');
  const rows=await sql`
   WITH payable AS (
    ...
    SELECT ${fulfillmentId},obligation_id,${requestId},${eventId},${runtime.actorId},quantity,unit,${PREVIEW_PICKUP_PLACE},'READY',now() FROM payable RETURNING *
   ), cmd AS (
    ...
    SELECT ${requestId},${commandId},'COMMITTED',${ownerToken},now(),1,jsonb_build_object('status','READY','obligationId',obligation_id,'fulfillmentId',fulfillment_id,'eventIds',jsonb_build_array(${eventId}::text)),now(),now() FROM f RETURNING command_id
   ), ver AS (
    ...
    SELECT ${eventId},f.obligation_id,v.version,'FULFILLMENT_READY',jsonb_build_object('operatorId',${runtime.actorId}::text,'quantity',jsonb_build_object('amount',f.ready_quantity,'unit',f.unit),'pickupPlace',f.pickup_place,'authorizedCommandId',${commandId}::text,'environment',${runtime.environment}::text),f.ready_at FROM f JOIN ver v ON v.aggregate_id=f.obligation_id RETURNING event_id
   )
