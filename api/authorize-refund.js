...
export default async function handler(req,res){
 ...
 try{
  ...
  const remedyId=`preview:remedy:${randomUUID()}`,eventId=`preview:event:${randomUUID()}`,commandId=`preview:command:${randomUUID()}`;
  const rows=await sql`
   WITH source AS (
    ...
    SELECT ${requestId},${commandId},'COMMITTED','vercel-preview',now(),1,jsonb_build_object('status','AUTHORIZED','obligationId',obligation_id,'remedyId',remedy_id,'eventIds',jsonb_build_array(${eventId}::text)),now(),now() FROM remedy RETURNING command_id
   ), ver AS (
    ...
    SELECT ${eventId},r.obligation_id,v.version,'REMEDY_AUTHORIZED',jsonb_build_object('operatorId',${OPERATOR_ID}::text,'remedyId',r.remedy_id,'sourceExceptionId',r.source_exception_id,'kind',r.kind,'quantity',jsonb_build_object('amount',r.quantity,'unit',r.unit),'amount',jsonb_build_object('minor',r.amount_minor,'currency',r.currency),'authorizedCommandId',${commandId}::text,'environment','preview'),r.authorized_at FROM remedy r JOIN ver v ON v.aggregate_id=r.obligation_id RETURNING event_id
   )
