...
export default async function handler(req,res){
 ...
 const principal=requirePreviewApiAuth(req,res,'member:fulfillment.accept',PARTICIPANT_ID);if(!principal)return;
 ...
 try{
  ...
  const rows=await sql`
   WITH target AS (
    ...
    UPDATE aggregate_version av SET version=av.version+(CASE WHEN ${shortfall}>0 THEN 2 ELSE 1 END) FROM upd u WHERE av.aggregate_id=u.obligation_id RETURNING av.aggregate_id,av.version AS final_version
   ), ev1 AS (
   ...
   SELECT u.obligation_id,u.state,u.accepted_quantity,u.shortfall_quantity,u.acceptance_event_id FROM upd u JOIN cmd ON true JOIN ev1 ON ev1.event_id=u.acceptance_event_id WHERE ${shortfall}=0 OR EXISTS (SELECT 1 FROM ev2 WHERE event_id=${exceptionEventId})`;
