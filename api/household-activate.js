...
/** J14: redeem a single-use household invitation into an ACTIVE/CURRENT beneficiary membership. */
export default async function handler(req,res,options={}){
  ...
  try{
    ...
    if(String(invitation.state)!=='APPROVED_PENDING_ACTIVATION')return res.status(409).json({ok:false,error:'HOUSEHOLD_INVITATION_NOT_ACTIVATABLE'});
    ...
    if(sponsors.length!==1||String(sponsors[0].state)!=='ACTIVE'||String(sponsors[0].standing)!=='CURRENT'||String(sponsors[0].member_type)!=='PRIMARY')return res.status(409).json({ok:false,error:'SPONSOR_MEMBERSHIP_NOT_CURRENT'});
    ...
    if(bindings.length===1&&String(bindings[0].state)!=='ACTIVE')return res.status(403).json({ok:false,error:'APPLICATION_PRINCIPAL_DISABLED'});
    ...
    const claimed=await sql`UPDATE household_beneficiary_invitation SET token_hash=NULL WHERE invitation_id=${String(invitation.invitation_id)} AND state='APPROVED_PENDING_ACTIVATION' AND token_hash=${tokenHash} AND expires_at>now() RETURNING invitation_id`;
    if(claimed.length!==1)return res.status(409).json({ok:false,error:'HOUSEHOLD_INVITATION_ALREADY_REDEEMED'});
    ...
    await sql`INSERT INTO application_membership(membership_id,participant_id,state,member_type,standing,sponsoring_membership_id,public_member_id,established_at,activated_at,eligibility_policy_version,eligibility_evidence_ids) VALUES (${membershipId},${participantId},'ACTIVE','HOUSEHOLD_BENEFICIARY','CURRENT',${sponsorId},${publicMemberId},${nowIso},${nowIso},'household-beneficiary-v1',${[String(invitation.invitation_id),sponsorId]})`;
    const activated=await sql`UPDATE household_beneficiary_invitation SET state='ACTIVE',beneficiary_membership_id=${membershipId},activated_at=${nowIso},activated_by_participant_id=${participantId} WHERE invitation_id=${String(invitation.invitation_id)} AND state='APPROVED_PENDING_ACTIVATION' RETURNING invitation_id`;
    if(activated.length!==1)return res.status(409).json({ok:false,error:'HOUSEHOLD_ACTIVATION_STATE_RACE'});
    ...
    res.setHeader('Cache-Control','no-store');return res.status(200).json({ok:true,invitationId:String(invitation.invitation_id),membershipId,publicMemberId,membershipState:'ACTIVE',standing:'CURRENT',memberType:'HOUSEHOLD_BENEFICIARY'});
