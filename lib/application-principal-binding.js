...
export async function resolveApplicationPrincipal(identity, requiredScope, options={}){
  ...
  try{
    ...
    if(requiredScope.startsWith('member:')){
      ...
      // parent_grant_id is provenance only (who authorized this grant, under what
      // authority) -- deliberately NOT walked here. A grant's own validity never
      // depends on its grantor's current status, so revoking an Admin's own grant
      // does not cascade-revoke everyone they delegated to.
      ...
      // A live, unexpired, unrevoked employee session is required in addition to
      // the grant itself -- "verify again" step-up, independently revocable
      // (Lock employee workspace / an Admin revocation) without touching the
      // underlying grant or the person's ordinary Google sign-in.
      const employeeSessionSecret=options.employeeSessionSecret || process.env.EMPLOYEE_SESSION_SECRET;
      if(typeof employeeSessionSecret!=='string' || employeeSessionSecret.length<32) return fail(503,'EMPLOYEE_SESSION_NOT_CONFIGURED');
      const sessionId=verifyEmployeeSessionTokenShape(employeeSessionSecret, options.employeeSessionToken);
      if(!sessionId) return fail(401,'EMPLOYEE_SESSION_REQUIRED');
      const sessions=await sql`
        SELECT session_id
         ...
         LIMIT 1`;
      if(sessions.length!==1) return fail(401,'EMPLOYEE_SESSION_INVALID');
      employeeSessionId=sessionId;
    }else{
      return fail(403,'AUTHORIZATION_SCOPE_FAMILY_UNSUPPORTED');
    }
