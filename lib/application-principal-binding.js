...
import { resolveMemberAccess } from './member-access-policy.js';
...
export async function resolveApplicationPrincipal(identity,requiredScope,options={}){
 ...
 try{let sql=options.sql;if(!sql){const {neon}=await import('@neondatabase/serverless');sql=neon(connectionString,{fetchOptions:{signal:AbortSignal.timeout(3000)}});}
  ...
  if(String(requiredScope).startsWith('member:')){
   ...
   const memberships=await sql`SELECT membership_id,state,standing FROM application_membership WHERE participant_id=${participantId} AND state='ACTIVE' LIMIT 2`;
   if(memberships.length!==1)return fail(403,'ACTIVE_MEMBERSHIP_REQUIRED');
   const standing=String(memberships[0].standing)==='CURRENT'?'ACTIVE':String(memberships[0].standing);
   const access=resolveMemberAccess({membershipStanding:standing,authenticated:true});if(!access.memberArea)return fail(403,'MEMBERSHIP_ACCESS_RESTRICTED');membershipId=String(memberships[0].membership_id);
  }else if(isWorkforceScope(requiredScope)){
   // Workforce authority is a separate state machine. An active authority grant plus
   // fresh employee step-up controls workforce access; paid-invoice standing does not.
