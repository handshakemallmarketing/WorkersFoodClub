...
export function requiredMinor(totalMinor,bps){
 ...
 if(!Number.isInteger(bps)||bps<1||bps>10000)throw new TypeError('bps must be an integer between 1 and 10000');
 return Math.ceil((totalMinor*bps)/10000);
}

function requireDemandQualificationBps(demandPoolBps){
 if(!Number.isInteger(demandPoolBps)||demandPoolBps<1||demandPoolBps>10000)throw new TypeError('demandPoolBps must be an explicit integer between 1 and 10000');
 return demandPoolBps;
}

export function qualificationState({totalMinor,paidMinor,demandPoolBps}){
 const governedBps=requireDemandQualificationBps(demandPoolBps);
 ...
 if(paid>=requiredMinor(total,governedBps))return 'DEMAND_QUALIFIED';
 return 'UNQUALIFIED';
}
...
export function deadlineDisposition({totalMinor,paidMinor,deliveryAt,now=new Date(),appliedToFulfillmentMinor=0,demandPoolBps}){
