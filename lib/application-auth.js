 ...
 * Production access is additionally guarded by an explicit default-deny
 * application kill switch. When enabled, Production first verifies OIDC
 * identity/provider claims and then resolves the canonical application actor
 * and governed scopes from the server-side application identity binding store.
 * IdP actor/scopes are not authoritative application bindings.
 */
export async function requireApplicationAuth(
  ...
  const bound = await resolver(
    ...
    requiredScope,
    {
      employeeSessionToken: extractEmployeeSessionToken(req),
      ...options.binding,
    },
  );
