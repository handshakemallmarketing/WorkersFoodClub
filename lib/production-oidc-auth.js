...
function scopesFromClaim(claim) {
...
export function readProductionOidcConfig(env = process.env) {
  ...
  const actorClaim = env.OIDC_ACTOR_CLAIM;
  const scopeClaim = env.OIDC_SCOPE_CLAIM || 'scope';

  if (![issuer, audience, jwksUri, actorClaim].every((v) => typeof v === 'string' && v.length > 0)) {
  ...
  return Object.freeze({
    ok: true,
    config: Object.freeze({ issuer, audience, jwksUri, actorClaim, scopeClaim }),
  });
}
...
export async function verifyProductionOidcToken({
  ...
  if (!header || !claims) return fail(401, 'TOKEN_INVALID');
  ...
  const actorId = claims[config.actorClaim];
  if (typeof actorId !== 'string' || actorId.length === 0) {
    return fail(403, 'APPLICATION_ACTOR_BINDING_REQUIRED');
  }

  const scopes = scopesFromClaim(claims[config.scopeClaim]);
  if (!scopes.includes(requiredScope)) return fail(403, 'AUTHORIZATION_SCOPE_REQUIRED');
  if (expectedActorId !== undefined && actorId !== expectedActorId) {
    return fail(403, 'AUTHENTICATED_ACTOR_MISMATCH');
  }
