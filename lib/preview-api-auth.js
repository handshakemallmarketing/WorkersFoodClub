...
/**
 * Allowlist, not a denylist: an unset or unexpected VERCEL_ENV value (typo,
 * misconfiguration, a platform other than Vercel) must fail closed rather than
 * silently behaving like preview. Only Vercel's own non-production values pass.
 */
export function isPreviewLikeEnvironment(env = process.env) {
...
export function verifyPreviewApiToken(req, requiredScope, now = Date.now()) {
  if (!isPreviewLikeEnvironment()) {
    return Object.freeze({
      ok: false,
      status: 403,
      error: 'PREVIEW_AUTH_PREVIEW_ONLY'
    });
  }
...
export function requirePreviewApiAuth(
  ...
  const actorAllowed = expectedActorId === undefined
    || (Array.isArray(expectedActorId)
      ? expectedActorId.includes(result.principal.actorId)
