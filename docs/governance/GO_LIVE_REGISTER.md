...
| **Canonical Vercel project** | — | **Clarified 2026-09-18.** `workers-food-club.vercel.app` (OriginOS team, `prj_Ti1NOQryzFeImhMSwncMXNxs01I8`) is the one real project — confirmed both by Willie directly and by matching its live deployment history to every commit tracked in this engagement. `workers-food-club-chi.vercel.app` is **not** an orphaned domain — the old workflow's hardcoded `VERCEL_ORG_ID`/`VERCEL_PROJECT_ID` (`team_wnL0jYsouqFvJ8VJR0jIAhTl` / `prj_Y10EuTs8ttzcRg9LZl2BASSiOFUE`) are real, different IDs from a Vercel account this session has no access to. **This means it cannot be deleted or even inspected from here** — that requires logging into whichever Vercel account owns `team_wnL0jYsouqFvJ8VJR0jIAhTl` directly. `PRODUCTION_APPLICATION_ACCESS_ACTIVATION-v1.json` has been updated (grant v4, superseding v3) to stop naming it canonical; the repo-side references are cleaned up. The actual stray Vercel project, if you find it, is yours to delete. |
