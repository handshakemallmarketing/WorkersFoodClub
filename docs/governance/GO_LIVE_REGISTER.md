...
| Production application-access **activation workflow** | `.github/workflows/production-application-access-activation.yml` + `sw1-production-application-access-activation-gate` branch | **Disconnected from reality.** Fired 10 times historically, but the branch's own copy of the workflow is pinned to `ACTIVATION_SHA=81ca013cc8f6706bc4aeee6669345ff1fb7b3755` — a candidate `PRODUCTION_APPLICATION_ACCESS_ACTIVATION-v1.json` itself describes as already rolled back — and it targets `https://workers-food-club-chi.vercel.app`, not the project that's actually enabled. **The real enablement above happened outside this workflow entirely** (direct Vercel dashboard toggle, per Willie's testing request). This workflow is not currently the mechanism controlling the real switch. Needs a decision: retire it, or repoint/re-pin it to match the `workers-food-club` (OriginOS) project and current `main`, and use it for future changes instead of direct dashboard toggles. |
| **Canonical Vercel project** | — | **Needs cleanup.** Two hostnames exist: `workers-food-club.vercel.app` (OriginOS team, confirmed real, confirmed intentionally enabled, running current `main`) and `workers-food-club-chi.vercel.app` (named as canonical in the stale governance JSON, returns `404 Deployment not found` via the OriginOS-scoped Vercel connection — could be a different Vercel account or an orphaned/unclaimed domain, can't tell which from this session). Willie: "there should just be one `workers-food-club`." Action needed: check your Vercel account(s) for a second project/team behind `-chi` and delete or reclaim it so only one project remains. |
