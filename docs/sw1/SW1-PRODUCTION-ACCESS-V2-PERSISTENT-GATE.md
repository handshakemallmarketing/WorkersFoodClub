...
Replace deployment-time dependence on short-lived human Google ID tokens with durable, governed Production application-access configuration while preserving request-time OIDC/JWKS authentication and server-side application principal authorization.
...
5. Runtime Production proves the public fail-closed boundary without privileged human credentials: switch OFF => protected route returns 503 `PRODUCTION_APPLICATION_ACCESS_DISABLED`; switch ON => unauthenticated protected route returns 401; malformed/invalid bearer credentials return 401.
